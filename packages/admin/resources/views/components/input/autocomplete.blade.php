<div x-data="{ showResults: false }" @click.outside="showResults = false">
  <div class="relative">
    {{--<x-hub::input.text @focus="showResults = true" {{ $attributes }} />--}}
      <x-hub::button size="sm" theme="gray" @click="showResults = true" {{ $attributes }}>
    ...
    -->
    @if($slot->isNotEmpty())
      <ul x-show="showResults" @click="showResults = false" class="absolute z-10 py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-3" x-cloak>
        <!--
